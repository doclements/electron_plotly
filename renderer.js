...
const nc = require('netcdf4');
const csv = require("fast-csv");
const plotly = require('plotly.js/dist/plotly.js');
let ncfile = new nc.File('gebco.nc','r');
...
let getLatLon = function(ncfile){
    ncfile = new nc.File(ncfile,'r');
    ...
    for (var x = 0; x < ncfile.root.dimensions['lat'].length ; x++) {
       //console.log("----------------------------------------------------");
       var line = []
       trace_bathy.y.push(ncfile.root.variables['lat'].read(x));
       for (var y = 0; y < ncfile.root.dimensions['lon'].length ; y++) {
          ...
          trace_bathy.x.push(ncfile.root.variables['lon'].read(y));
          
       }
    
       trace_bathy.z.push(line);
    }
    
}

$('#tester').html('<pre>'+ncfile.root.dimensions['lat'].length+'</pre>');



csv.fromPath("data/bathy_553/glider.csv")
.on("data", function(data){
    ...
    trace_overlay.x.push(data[2]);
    trace_overlay.y.push(data[1]);
    trace_overlay.z.push(5);
...
.on("end", function(){
    ...
    //       z:
    getLatLon('data/bathy_553/gebco.nc')
    //       type: "surface"
    //     }
    //   ];

      //var graphOptions = {layout: layout, filename: "merge mess", fileopt: "overwrite"};
      plotly.newPlot('tester', [trace1,trace_bathy,trace_overlay], layout);
    });
